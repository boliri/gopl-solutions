...
//
// in contrast with chatv1.go, this one is easier to reason about as it delegates the check for
// idle users to handleConn so users let the broadcaster know they're idle, and not the other way
package main
...
func handleConn(conn net.Conn) {
	...
	go func() {
		...
		for {
			<-ticker.C
			if cli.isIdle() {
				conn.(*net.TCPConn).CloseRead() // don't let the user type anything
